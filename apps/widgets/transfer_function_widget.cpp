...
void TransferFunctionWidget::draw_ui()
{
    ...
    std::transform(colormaps.begin(), colormaps.end(), std::back_inserter(comboText),
        [](const Colormap &c) { return c.name.c_str(); });

    int current_colormap = selected_colormap;
    if (ImGui::Combo("Colormap", &current_colormap, comboText.data(), comboText.size())) {
      selected_colormap = current_colormap;
      update_colormap();
    }
    ...
    draw_list->AddPolyline(polyline_pts.data(), polyline_pts.size(), 0xFFFFFFFF, false, 2.f, false);
