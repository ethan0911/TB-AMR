...
// This function generates graphical output like it is done in step-22.
  ...
  StokesProblem<dim>::output_results (const unsigned int refinement_cycle)  const
  {
    ...
    data_out.add_data_vector (estimated_error_per_cell, "kelly_u",
...
// respective order.
  ...
  void StokesProblem<dim>::run ()
  {
    ...
    for (unsigned int refinement_cycle = 0; refinement_cycle<17;
         ++refinement_cycle)
      {
...
public:
  virtual double value (const Point<dim> &p,
                        const unsigned int component = 0) const
  {
    auto func = [](const Point<dim> &p) -> double
    {
        double r = p.norm();
        double v = std::pow(sin(r*1.5*3.141),20.0);
        return v*1.5;
      };
    return func(p);
  }
...
template <int dim>
void surface_refine(Triangulation<dim> &tria)
{
  MyFunc<dim> func;
    ...
    if (cell->is_locally_owned())
      {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
          {
            value = round(func.value(fe_values.quadrature_point(q_point)));
            sum += value;
          }

        if (std::abs(sum/n_q_points - value)>1e-5)
...
template <int dim>
void output_results(parallel::distributed::Triangulation<dim> &tria)
{
  ...
  TrilinosWrappers::MPI::Vector solution(dof_handler.locally_owned_dofs());
  TrilinosWrappers::MPI::Vector locally_relevant_solution(dof_handler.locally_owned_dofs(),
...
void cube()
{
  ...
  for (unsigned int i=0;i<10;++i)
    {
      std::cout << tria.n_global_active_cells() << " cells..." << std::endl;
      surface_refine(tria);
      output_results<dim>(tria);
    }
